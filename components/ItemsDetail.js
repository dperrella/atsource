...
import {Modal, Text, View, TextInput,Platform,StyleSheet, ScrollView,ActivityIndicator, 
  Image, Alert, FlatList, TouchableOpacity,TouchableHighlight } from 'react-native';
...
var urlDelPhoto = appUrl+'ats/DelPhoto';
...
import okok from '../images/okok.png';
...
class ItemsDetail extends Component {
  ...
  setModalVisible(visible, item) {
    ...
    if(item==null)
      this.setState({modalVisible: visible, idphoto: -1, photo: null})
    else
      this.setState({modalVisible: visible, idphoto: item.image.id, photo: item.image.url});
  ...
  confirmDelete() {
    ...
    Alert.alert(
      'Confirm',
      'Confirm Delete Photo From Item',
      [
        {
          text: 'Cancel',
        ...
        {text: 'OK', onPress: () => {
            ...
            console.log(urlDelPhoto+'?'+formBody);
        
            fetch(urlDelPhoto+'?'+formBody, {
            ...
            */
            ...
            fetch(urlX+'?id='+this.state.idphoto+'&idItem='+this.state.itemId)
            .then(res => res.json())
            .then(res => {

                this.setState({
                  photoList: res.listings,
                  loading: false,
                  modalVisible: false,
                  photo: null
                });
  ...
  _save() {
    ...
    if(this._checkErr()) {
      ...
      for(i=0;i<this.state.selectedCustomers.length;i++) {
        console.log(this.state.selectedCustomers[i])
        sc+=this.state.selectedCustomers[i]+',';
      }
      ...
      .then((responseJson) => {
        ...
        this.setState({loading:false, insert: false, itemId: responseJson.idItem, showMessage: true});
    ...
    fetch(url+'?idItem='+this.props.itemId)
      ...
      .then(res => {
        ...
        this.setState({
          ...
          photoList: res.photo,
          selectedCustomers: res.customers,
          loading: false
        });
      })
      .catch(error => {
  ...
  openImage = (picked) => {
    ...
    this.setModalVisible(true, picked);
        ...
        <Modal
            ...
            <View>
              ...
              <View style={{
                ...
                <View style={{flex: 1,
                  ...
                  <View style={{width: '50%',backgroundColor: '#506D85',justifyContent: 'center',
                    alignItems: 'center',
                    height: 60,}}>
                    <TouchableHighlight
                      onPress={() => {
                      ...
                      fontSize:20,
                      fontWeight: '500', color:'#fff'}} >Close Image</Text>
                    </TouchableHighlight>
                  </View>

                  <View style={{width: '50%',backgroundColor: '#de5c56',justifyContent: 'center',
                    alignItems: 'center',
                    height: 60,}}>
                    <TouchableHighlight
                      onPress={() => {
                      ...
                      <Text style={{marginTop:10,
                      fontSize:20,
                      fontWeight: '500', color:'#fff'}} >Delete Image</Text>
                    </TouchableHighlight>
                  </View>
                </View>
              </View>
              <Image style={{width: DEVICE_WIDTH, height: DEVICE_HEIGHT-40}} source={{ uri:  this.state.photo}}></Image>
          ...
          <View style={styles.inputWrapper}>
            ...
            */}
            ...
            <TouchableOpacity style={styles.mySelect} onPress={() => {
              this.setState({ venvisible: true });
            }}>
              <Text style={{color: 'black',fontSize: 16}}>{this.state.vendorName}</Text>
            </TouchableOpacity>  

            <ModalFilterPicker
              visible={this.state.venvisible}
              onSelect={(picked) => {
          ...
          <View style={styles.inputWrapper}>
            ...
            */}
            ...
            <TouchableOpacity style={styles.mySelect} onPress={() => {
              this.setState({ bravisible: true });
            }}>
              <Text style={{color: 'black',fontSize: 16}}>{this.state.brandName}</Text>
            </TouchableOpacity>  

            <ModalFilterPicker
              visible={this.state.bravisible}
              onSelect={(picked) => {

                this.setState({
                  brand: picked.key,
                  brandName: picked.label,
                  bravisible: false
                })
              }}
              onCancel={() => {
          ...
          <View style={styles.inputWrapper}>
            ...
            */}
            ...
            <TouchableOpacity style={styles.mySelect} onPress={() => {
              this.setState({ catvisible: true });
            }}>
              <Text style={{color: 'black',fontSize: 16}}>{this.state.categoryName}</Text>
            </TouchableOpacity>  
          ...
          <View style={styles.inputWrapper}>
            ...
            */}
            ...
            <TouchableOpacity style={styles.mySelect} onPress={() => {
              this.setState({ couvisible: true });
            }}>
              <Text style={{color: 'black',fontSize: 16}}>{this.state.countryName}</Text>
            </TouchableOpacity>  

            <ModalFilterPicker
              visible={this.state.couvisible}
              onSelect={(picked) => {

                this.setState({
                  country: picked.key,
                  countryName: picked.label,
                  couvisible: false
                })
              }}
              onCancel={() => {
